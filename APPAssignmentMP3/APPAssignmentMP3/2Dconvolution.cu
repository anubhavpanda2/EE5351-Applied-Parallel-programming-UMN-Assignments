...
int main(int argc, char** argv) {
   ...
   // t=//(clock_t)
//t/CLOCKS_PER_SEC;
    printf("timetaken by gpu %lf",(double)t/CLOCKS_PER_SEC);
	...
	printf("timetaken by cpu%lf",(double)t/CLOCKS_PER_SEC);
...
//st=clock();
    ...
    dim3 dimGrid(ceil(N.width/(float)TILE_SIZE),ceil(N.height/(float)TILE_SIZE));

   // dim3 dimGrid(N.width/TILE_SIZE,N.height/TILE_SIZE);
   //
...
//cudaEventCreate(&stop);
    // Launch the device computation threads!ConvolutionKernel<<<dimGrid,dimBlock>>>(Matrix N, Matrix P)
...
printf("new GPU time %lf",time);
//cudaDeviceSynchronize() ;
//mid1=clock();
    //end=clock();
    //printf("timetaken kernel %lf",(double)(end-start));
...
//clock_t t=end-mid1;
//t=t+mid-st;
//printf("overhead%lf",(double)t/CLOCKS_PER_SEC);
...
//t=end-mid1;
//t=t+mid-st;
//printf("overhead%lf",(double)t/end=clock();
//clock_t t=end-mid1;
//t=t+mid-st;
//printf("overhead%lf",(double)t/CLOCKS_PER_SEC);
//);
