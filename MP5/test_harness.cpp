...
int main(int argc, char* argv[])
{
	...
	uint32_t * input_device=(uint32_t *) AllocateDataInDevice(INPUT_HEIGHT,INPUT_WIDTH, sizeof(uint32_t));
//printf("hi");
CopyFromHostToDevice(input_device,input, INPUT_HEIGHT, INPUT_WIDTH, sizeof(uint32_t));
uint32_t* device_bins=(uint32_t *) AllocateDataInDevice(HISTO_HEIGHT,HISTO_WIDTH, sizeof(uint32_t));
cudaMemset(device_bins , 0, HISTO_WIDTH * HISTO_HEIGHT * sizeof(uint32_t));    
/* End of setup code */

    /* This is the call you will use to time your parallel implementation */
    TIME_IT("opt_2dhisto",
            50,
            opt_2dhisto(input_device,INPUT_HEIGHT,INPUT_WIDTH,device_bins);)

    /* Include your teardown code below (temporary variables, function calls, etc.) */

CopyFromDeviceToHost(kernel_bins,device_bins,HISTO_HEIGHT*HISTO_WIDTH,sizeof(uint32_t));
FreeDeviceData(device_bins);
FreeDeviceData(input_device);
